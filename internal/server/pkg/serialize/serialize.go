...
type SerializeInterface interface {
	...
	SerializerResponse(metricsSData *Metrics) *string
	Errors() error
}
...
func (s *Serializer) SerializerResponse(metricsSData *Metrics) *string {

	var sendStringMetrics string

	serializeErr := s.SetData(metricsSData).GetData(&sendStringMetrics)
